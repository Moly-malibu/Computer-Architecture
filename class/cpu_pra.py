...
#CPU ON
  def run(self):
    ...
    while not self.halt: 
      ...
      self.operation_a = self.ram_read(self.PC+1)
      self.operation_b = self.ram_read(self.PC+2)
      self.pc_override = False
      bit_5 = (self.ir & 2**5) >>5
      if self.interrupt:
            ...
            interrupt_mask = self.reg[5] & self.reg[6]
            for i in range(8):
                  interrupted = ((interrupt_mask>>i)&1)==1
                  if interrupted:
                        self.ir= 0b01010010
      if bit_5:
            op = self.brach_table[self.ir]
            self.alu(op, self.operation_a, self.operation_b)
